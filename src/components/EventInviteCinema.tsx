          ...
          <div className="relative aspect-video bg-black">
            {playing && youtubeId ? (
              ...
              >
                ...
                <span className="absolute top-4 left-4 sm:top-6 sm:left-6 inline-flex items-center gap-2 rounded-full border border-[var(--ykp-gold)]/40 bg-black/40 px-3 py-1.5 backdrop-blur-sm">
                  <span className="h-1.5 w-1.5 rounded-full bg-[var(--ykp-gold)]" />
                  <span className="text-[10px] sm:text-[11px] font-semibold tracking-[0.22em] uppercase text-[var(--ykp-gold)]">
                    Official invitation
                  </span>
                </span>

                <span className="absolute inset-0 flex items-center justify-center">
                  <span className="relative flex h-[4.75rem] w-[4.75rem] sm:h-24 sm:w-24 items-center justify-center">
                    ...
                    <span className="relative flex h-16 w-16 sm:h-[4.5rem] sm:w-[4.5rem] items-center justify-center rounded-full bg-[var(--ykp-gold)] text-[var(--ykp-ink)] shadow-[0_12px_40px_rgba(201,162,39,0.45)] transition-transform duration-300 group-hover:scale-110">
                      <Play className="h-7 w-7 sm:h-8 sm:w-8 fill-current ml-1" />
                    </span>
                  </span>
                </span>

                <span className="absolute bottom-0 inset-x-0 p-5 sm:p-8">
                  <p className="font-display text-xl sm:text-3xl lg:text-4xl font-semibold text-white leading-tight text-balance">
                    {event.title}
                  </p>
                  {event.subtitle && (
                    <p className="mt-1 text-[var(--ykp-gold)] font-display text-base sm:text-xl">
                      {event.subtitle}
                    </p>
                  )}
                  <p className="mt-3 text-xs sm:text-sm text-white/70 tracking-wide">
                  ...
                  <span className="mt-4 flex flex-wrap items-center justify-between gap-2">
                    <span className="text-[10px] sm:text-[11px] font-semibold tracking-[0.2em] uppercase text-white/50">
                      YKP Foundation  ·  Pakistan&apos;s Digital Flight
                    </span>
                    <span className="text-[10px] sm:text-[11px] font-semibold tracking-[0.18em] uppercase text-[var(--ykp-gold)]">
